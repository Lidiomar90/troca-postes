...
function diagnosticoCompleto() {
  ...
  const abasObrigatorias = [SHEET.TROCAS, SHEET.CONFIG, SHEET.BASE_REDE,
                             SHEET.LOG_EXECUCAO, SHEET.LOG_ALERTAS, SHEET.FILA_REVISAO];
  abasObrigatorias.forEach(nome => {
    const sh = ss.getSheetByName(nome);
    if (sh) {
      resultados.push(['OK', `Aba "${nome}" existe (${sh.getLastRow()-1} linhas)`]);
    } else {
  ...
  if (shTrocas && shTrocas.getLastRow() > 1) {
    ...
    const semGeo = dados.filter(r => r[COL.GEO_STATUS] !== 'OK' && r[COL.STATUS] !== 'CANCELADO').length;
    const semRede = dados.filter(r => !r[COL.REDE_STATUS] && r[COL.GEO_STATUS] === 'OK').length;
  ...
  try {
    ...
    if (resp.getResponseCode() === 200) {
      ...
      resultados.push(['OK', `GitHub: repositório "${ghRepo}" acessível (${repo.visibility})`]);
    } else {
  ...
  shLog.appendRow([new Date(), 'DIAGNOSTICO', `RESUMO: ${totalOk} OK, ${totalAviso} AVISO, ${totalErro} ERRO`, totalErro > 0 ? 'ERRO' : totalAviso > 0 ? 'AVISO' : 'OK']);

  // Destaca erros em vermelho no log
  const lastRow = shLog.getLastRow();
  if (totalErro > 0) {
    const erroRows = resultados.reduce((acc, r, i) => r[0] === 'ERRO' ? [...acc, lastRow - resultados.length + i] : acc, []);
  ...
  const summary = `${icon} Diagnóstico concluído\n\n` +
    `✅ OK: ${totalOk}\n⚠️ Avisos: ${totalAviso}\n❌ Erros: ${totalErro}\n\n` +
    (totalErro > 0 ? 'ERROS ENCONTRADOS:\n' + resultados.filter(r=>r[0]==='ERRO').map(r=>'• '+r[1]).join('\n') : '') +
...
function diagnosticoRapido() {
  ...
  [PROP.TELEGRAM_TOKEN, PROP.TELEGRAM_CHATID].forEach(k => {
    if (!props.getProperty(k)) erros.push(`Script Property ausente: ${k}`);
  });

  const ss = getSpreadsheet();
  if (!ss.getSheetByName(SHEET.TROCAS)) erros.push('Aba TROCAS não existe');
